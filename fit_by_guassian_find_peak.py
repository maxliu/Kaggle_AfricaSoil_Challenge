...
window = signal.general_gaussian(50, p=0.5, sig=20)
filtered = signal.fftconvolve(window, data)
filtered = (np.average(data) / np.average(filtered)) * filtered
filtered = np.roll(filtered, -25)


peakidx = signal.find_peaks_cwt(filtered, np.arange(10,50))
print peakidx


fig2 = pl.figure()
pl.plot(y_vals)
pl.plot(peakidx,[filtered[i] for i in peakidx],'ro')
pl.plot(filtered)
pl.show()

fig3 = pl.figure()
pl.plot(x,y_vals)
pl.plot([x[i] for i in peakidx],[y_vals[i] for i in peakidx],'ro')
pl.show()
...
##results, value = fmin_slsqp(err, params, args=(x, y_vals),iter=20000) # this is not working 
#
